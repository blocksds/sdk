...
// each tile.
//
// For this example, the city1.png image is converted to use palette 12, and the
// city2.png image is converted to use palette 7. Check the ".grit" files for
// more details.
//
...
int main(int argc, char *argv[])
{
    ...
    dmaCopy(city1Tiles, bgGetGfxPtr(bg0), city1TilesLen);
    dmaCopy(city1Map, bgGetMapPtr(bg0), city1MapLen);
    dmaCopy(city1Pal, &VRAM_E_EXT_PALETTE[0][12], city1PalLen);

    int bg1 = bgInit(1, BgType_Text8bpp, BgSize_T_256x256, 4, 4);

    dmaCopy(city2Tiles, bgGetGfxPtr(bg1), city2TilesLen);
    dmaCopy(city2Map, bgGetMapPtr(bg1), city2MapLen);
    dmaCopy(city2Pal, &VRAM_E_EXT_PALETTE[1][7], city2PalLen);
    ...
    dmaCopy(city1Tiles, bgGetGfxPtr(bg1sub), city1TilesLen);
    dmaCopy(city1Map, bgGetMapPtr(bg1sub), city1MapLen);
    dmaCopy(city1Pal, &VRAM_H_EXT_PALETTE[1][12], city1PalLen);

    int bg2sub = bgInitSub(2, BgType_Text8bpp, BgSize_T_256x256, 4, 5);

    dmaCopy(city2Tiles, bgGetGfxPtr(bg2sub), city2TilesLen);
    dmaCopy(city2Map, bgGetMapPtr(bg2sub), city2MapLen);
    dmaCopy(city2Pal, &VRAM_H_EXT_PALETTE[2][7], city2PalLen);

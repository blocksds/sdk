...
int main(int argc, char *argv[])
{
    ...
    //
    // forest_townMetaMap[] is a 16x16 map in which each entry is a metatile
    // index.
    //
    // forest_townMetaTiles[] is an array of metatiles. There are 2x2 tiles in
    // each metatile. If you want to access the 4 tiles of a specific metatile
    // you need to access the 4 bytes starting at "index * 4".
    ...
    for (u32 ty = 0; ty < 32; ty++)
    {
        for (u32 tx = 0; tx < 32; tx++)
        {
            ...
            u16 meta_map_entry = forest_townMetaMap[my * metamap_width + mx];

            // meta_map_entry is the metatile index

            const u16 *meta_tile = &forest_townMetaTiles[meta_map_entry * metatile_size];

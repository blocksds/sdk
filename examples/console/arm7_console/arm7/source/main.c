...
int main(int argc, char *argv[])
{
    ...
    while (!exit_loop)
    {
        ...
        // Note that consolePrintf() only supports the following flags (with no
        // modifiers): %c %d %s %u %x. All integers are 32 bit in size.
        consolePuts("ARM7: ");
        consolePrintf("%d (%c) %s\n", count, c, condition ? "true" : "false");

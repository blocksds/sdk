...
def generate_input():
    ...
    yield from repeat(JoypadState(select=True), 3)
    yield from repeat(JoypadState(select=False), 2)
    yield from repeat(JoypadState(left=True, up=True, r=True, x=True), 20)

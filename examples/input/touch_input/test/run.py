...
def generate_input():
    ...
    yield from repeat(Pointer(*bt.touch(10, 10), True), 5)
    yield from repeat(0, 5)

    yield from repeat(Pointer(*bt.touch(10, 191-10), True), 5)
    yield from repeat(0, 5)

    yield from repeat(Pointer(*bt.touch(255-10, 191-10), True), 5)
    yield from repeat(0, 5)

    yield from repeat(Pointer(*bt.touch(255-10, 10), True), 5)
    yield from repeat(0, 5)

    yield from repeat(Pointer(*bt.touch(127, 91), True), 5)
    yield from repeat(0, 5)
...
with bt.session_start(bt.find_rom(), generate_input, log_driver) as session:
    bt.session_run_frames(session, 15)

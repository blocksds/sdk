...
def session_start(game, input_gen=None, log_driver=None, user_options=None, *,
    ...
    testdir = path_driver.root_dir.decode("utf-8")
    system_dir = path_driver.system_dir.decode("utf-8")
    playlist_dir = path_driver.playlist_dir.decode("utf-8")
    save_dir = path_driver.save_dir.decode("utf-8")
    save_directory = save_dir
    savestate_directory = os.path.join(testdir, 'states')
    ...
    if not os.path.exists(os.path.join(system_dir, 'nand.bin')):
        os.symlink(os.path.join(blobs_path, 'nand.bin'), os.path.join(system_dir, 'nand.bin'))
        os.symlink(os.path.join(blobs_path, 'dsfirmware.bin'), os.path.join(system_dir, 'dsfirmware.bin'))
        os.symlink(os.path.join(blobs_path, 'bios7i.bin'), os.path.join(system_dir, 'dsi_bios7.bin'))

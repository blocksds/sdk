...
with bt.session_start(bt.find_rom(), generate_input, log_driver) as session:
    ...
    res_1 = bt.save_screenshot(session, 'build/result_1.png')

    bt.session_run_frames(session, 30)
    res_2 = bt.save_screenshot(session, 'build/result_2.png')

    bt.session_run_frames(session, 30)
    res_3 = bt.save_screenshot(session, 'build/result_3.png')

    bt.assert_no_errors_in_logs(log_driver)

bt.compare_image_with_reference(res_1, 'test/reference_1.png')
bt.compare_image_with_reference(res_2, 'test/reference_2.png')
bt.compare_image_with_reference(res_3, 'test/reference_3.png')

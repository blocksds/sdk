...
// This example is a stability test of the CMD/REPLY commands in an application
// using local multiplayer communications. It sends messages between all
// consoles every frame.
//
// Each console keeps a counter that increments every frame. The clients send
// their counter to the host, and the host sends the count of every console to
// the clients. The clients only print on the screen the values received by the
// host.
//
// Leave the consoles on while the counters count, and check them every now and
// then to make sure that it doesn't hang.
...
void RenderGameState(void)
{
    for (int i = 0; i < WIFI_MAX_MULTIPLAYER_CLIENTS + 1; i++)
    {
        ...
        consoleSetCursor(NULL, 0, i + 5);
        printf("%lu", game.player[i].counter);
    }
}
...
void SendHostStateToClients(void)
{
    ...
    for (int i = 0; i < WIFI_MAX_MULTIPLAYER_CLIENTS + 1; i++)
    {
        if (game.player_mask & BIT(i))
        {
            host_packet.player[i].counter = game.player[i].counter;
        }
        else
        {
            host_packet.player[i].counter = 0;
        }
    }
...
void FromHostPacketHandler(Wifi_MPPacketType type, int base, int len)
{
    ...
    for (int i = 0; i < WIFI_MAX_MULTIPLAYER_CLIENTS + 1; i++)
    {
        game.player[i].counter = packet.player[i].counter;
    }
...
void FromClientPacketHandler(Wifi_MPPacketType type, int aid, int base, int len)
{
    ...
    game.player[aid].counter = packet.counter;
...
connect:
    ...
    while (1)
    {
        ...
        // Prepare new packet to be sent to the host
